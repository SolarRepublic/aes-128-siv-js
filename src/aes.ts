...
// perform AES-CBC
export const aes_cbc = async(d_key_cbc: CryptoKey, atu8_data: Uint8Array): Promise<Uint8Array> => new Uint8Array(await crypto.subtle.encrypt({
	name: 'AES-CBC',
	iv: ATU8_ZERO_BLOCK.slice(),
}, d_key_cbc, atu8_data), 0, NB_AES_BLOCK);

// perform AES-CTR
export const aes_ctr = async(d_key_ctr: CryptoKey, atu8_iv: Uint8Array, atu8_data: Uint8Array): Promise<Uint8Array> => new Uint8Array(await crypto.subtle.encrypt({
	name: 'AES-CTR',
	counter: atu8_iv,
	length: NB_AES_BLOCK,
}, d_key_ctr, atu8_data));
...
export const aes_cmac_init = async(d_key_mac: CryptoKey): Promise<(atu8_data: Uint8Array) => Promise<Uint8Array>> => {
	...
	return async(atu8_data: Uint8Array): Promise<Uint8Array> => {
		...
		// last block
		const atu8_last = new Uint8Array(NB_AES_BLOCK);
...
export const s2v = async(d_key_rkd: CryptoKey, atu8_plaintext: Uint8Array, a_ad=[ATU8_NIL]): Promise<Uint8Array> => {
	...
	const atu8_sn = new Uint8Array(NB_AES_BLOCK);
